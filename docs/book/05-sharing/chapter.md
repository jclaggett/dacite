...
Bob now contacts the server and says, "I would like to claim the share named `photos` from Alice." The server looks up Alice's current root, finds the `shares` entry for `photos`, checks whether Bob is in the authorized set, and if so, adds the target hash `#T` to the set of roots Bob is allowed to use. From that moment on, Bob can issue GET and PUT requests against `#T` using the normal proof chain mechanism. His access is read-only on Alice's tree because he does not have a chain proof that would let him modify it.

The sequence is straightforward once you see it in action. Alice offers. Bob claims. The server validates the claim against Alice's current root and grants Bob a new valid root for the duration of his session. No special server state is required beyond what is already stored in Alice's root.
