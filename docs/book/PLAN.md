...
The book introduces content stores before hash for pedagogy; in code, `dacite.store` and `dacite.hash` are peers at the bottom of the stack. Rooted stores live in `dacite.rooted`; LRU and remote stores in `dacite.store.lru` and `dacite.store.remote`.
...
| 4 | Rooted stores | `dacite.rooted` (→ `dacite.rooted.gc` for GC) |
